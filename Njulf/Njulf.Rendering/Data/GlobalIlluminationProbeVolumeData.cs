...
/// <summary>
/// Compatibility conversion for authored GI volumes. Resident V2 may build
...
public static class GlobalIlluminationProbeVolumeData
{
    ...
    public const ulong AtlasBytesPerProbe = IrradianceBytesPerProbe + VisibilityBytesPerProbe;
        ...
        int reservedPhysicalProbeCount = 0)
    {
        ...
        for (int i = 0; i < authoredVolumes.Count && written < destination.Length; i++)
        {
            ...
            destination[written] = BuildGpuVolume(
                volume,
                firstProbeIndex,
                metadata,
                settings,
                volumeRaysPerProbe,
                volumeMaxProbeUpdatesPerFrame);
            written++;
        }
        ...
        maxProbeUpdatesPerFrame = Math.Min(
            Math.Min(maxProbeUpdatesPerFrame, settings.SimpleDdgiProbeUpdatesPerFrame),
            activeProbeCount);
        return written;
    }
        ...
        DdgiRuntimeWarmupState cacheWarmupState = DdgiRuntimeWarmupState.Disabled)
    {
        ...
        return new GPUDdgiProbeVolumeHeader
        {
            ...
            ThinWallLeakClampStrength = settings.DdgiThinWallPolicyEnabled
                ? settings.DdgiThinWallLeakClampStrength
                : 0.0f,
            ThinWallProxyThickness = settings.DdgiThinWallPolicyEnabled
    ...
    public static ulong EstimateIrradianceAtlasBytes(int probeCount) =>
        EstimateProbeAtlasBytes(probeCount, IrradianceTexelsPerProbe, Rgba16FloatBytesPerTexel);

    public static ulong EstimateVisibilityAtlasBytes(int probeCount) =>
        EstimateProbeAtlasBytes(probeCount, VisibilityTexelsPerProbe, Rg16FloatBytesPerTexel);

    public static ulong EstimateTextureBytes(int probeCount) =>
        checked(EstimateIrradianceAtlasBytes(probeCount) + EstimateVisibilityAtlasBytes(probeCount));
    ...
    public static int CalculateAtlasProbeCapacity(ulong atlasBudgetBytes)
    {
        ...
        return capacity > GlobalIlluminationSettings.MaxSimpleDdgiTotalProbeCount
    ...
    public static int CalculateActiveProbeBudget(GlobalIlluminationSettings settings)
    {
        ...
        return Math.Clamp(
            Math.Min(GlobalIlluminationSettings.MaxSimpleDdgiTotalProbeCount, atlasProbeBudget),
            0,
            GlobalIlluminationSettings.MaxSimpleDdgiTotalProbeCount);
    }
        ...
        int maxProbeUpdatesPerFrame)
    {
        ...
        return new GPUDdgiProbeVolume
        {
            ...
            RayAndUpdateParams = new Vector4(
                raysPerProbe,
                Math.Min(maxProbeUpdatesPerFrame, settings.SimpleDdgiProbeUpdatesPerFrame),
                volume.Intensity,
                hysteresis),
    ...
    private static float EffectiveDdgiSelfShadowBias(float authoredBias, GlobalIlluminationSettings settings) =>
        Math.Clamp(authoredBias * settings.DdgiSelfShadowBiasScale, 0.0f, 10.0f);
    ...
    private static float EffectiveDdgiHysteresis(float authoredHysteresis, GlobalIlluminationSettings settings)
    {
        ...
        float scaledBlendAlpha = Math.Clamp(baseBlendAlpha, 0.0f, 1.0f);
        ...
        DdgiProbeVolumeRuntimeMetadata metadata)
    {
        ...
        // Authored volumes retain their own ray cardinality. Automatic rings
        // can still pass a cascade hint, but the compatibility conversion is
        // deliberately independent of the resident scheduler's ring policy.
        _ = cascadeIndex;
        return Math.Clamp(
            volume.RaysPerProbe,
            GlobalIlluminationSettings.MinSimpleDdgiRaysPerProbe,
    ...
    private static float EffectiveMaxRayDistance(GlobalIlluminationProbeVolume volume) =>
        MathF.Max(volume.MaxRayDistance, 0.1f);
