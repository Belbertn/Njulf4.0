...
## Round-4 confirmation: the artifacts are grid-phase-dependent (they "breathe" as the clipmap scrolls)

Four paired shots (normal + FullSlice, two positions) on the same `6cdff10` build:

- Position A (spawn): floor now renders **smooth** in FullSlice where round 3 showed heavy acne; position B (~same spot as round 3's sealed shed): the doorway is now **open**, but the mid-corridor (10–25 m, cascade-1/2/3 territory) is full of floating dark blobs, pink 0.25–1 m "rubble" on the floor, and a green c3 backdrop — the same inflation at coarser voxel scale.
- Explanation: the buggy predicate's flip decision depends on where surfaces sit relative to the voxel grid. A floor whose first voxel shell has centers ~0.56 voxel above the surface doesn't trigger (half-voxel probe stays positive); ~0.2 voxel above, it flips → relief. As cascades scroll (camera movement, Y drift), the grid phase against each surface changes, so acne/membranes appear and disappear at the same world locations. That is why the same build produced a sealed doorway at 02:34 and an open one at 02:54.
- The normal-render pair shows the GI cost: soft white splotches on the floor near the green wall (probe rays hitting phantom inflated surfaces just above geometry) and the persistent surface-cache fallback elevation (7–16% vs <1% baseline, `RejectDepthUv` 275–600).

This removes any remaining doubt: one mechanism (over-broad flip predicate + trace-side near-minimum hits), phase-modulated by clipmap scroll, accounts for the acne, the rubble, the membranes, their flicker over time, the fallback spike, and the 2.6–4.6 ms brick cost.
