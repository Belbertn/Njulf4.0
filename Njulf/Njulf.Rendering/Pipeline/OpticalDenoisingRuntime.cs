...
internal sealed unsafe class OpticalDenoisingRuntime : IDisposable
{
    ...
    public ulong AllocatedBytes => _bytes * 2 + (_readback[0].IsValid ? 64UL : 0UL);
    ...
    private void EnsureResources()
    {
        ...
        if (_width == extent.Width && _height == extent.Height && _budget == _settings.OpticalDenoising.MemoryBudgetMiB) return;
        ...
        try
        {
            ...
            var allocation = OpticalDenoisingGpuContract.Allocation(_width, _height, _budget, _context.MaximumStorageBufferRange);
    ...
    public void Begin(CommandBuffer cmd, int frameIndex, SceneRenderingData scene)
    {
        ...
        ulong prefix = (OpticalDenoisingGpuContract.HeaderWords + (ulong)_width * _height * OpticalDenoisingGpuContract.PixelWords) * 4;
        ...
        header[4] = (uint)_settings.OpticalDenoising.LayerLimit;
    ...
    public void Record(CommandBuffer cmd, int frameIndex, SceneRenderingData scene, int stage)
    {
        ...
        if (stage == 1) Dispatch(cmd, _temporal, push);
        else if (stage == 2)
        {
            push[2] = 1; Dispatch(cmd, _spatial, push);
            Barrier(cmd, PipelineStageFlags2.ComputeShaderBit, AccessFlags2.ShaderStorageWriteBit,
                PipelineStageFlags2.ComputeShaderBit, AccessFlags2.ShaderStorageReadBit | AccessFlags2.ShaderStorageWriteBit);
            push[2] = 2; Dispatch(cmd, _spatial, push);
        }
        else
        {
    ...
    private void Dispatch(CommandBuffer cmd, VkPipeline pipeline, uint* push)
    {
        ...
        _context.Api.CmdDispatch(cmd, (_width + 7) / 8, (_height + 7) / 8, 1);
    }
    ...
    private void DestroyBuffers()
    {
        ...
        _bytes = 0; _historyValid = false; _active = false; _previousBank = -1;
    }
    ...
    public void Dispose()
    {
        ...
        _disposed = true;
        DestroyBuffers();
        foreach (VkPipeline pipeline in new[] { _temporal, _spatial, _correct })
