    ...
    public unsafe class VulkanContext : IDisposable
    {
        ...
        private void LoadDebugUtils()
        {
            if (!_vk.TryGetInstanceExtension(_instance, out _extDebugUtils))
                throw new VulkanException("VK_EXT_debug_utils was enabled but could not be loaded.");
        }

        private void SetupDebugMessenger()
        {
            if (_extDebugUtils == null)
                throw new VulkanException("VK_EXT_debug_utils must be loaded before creating the validation messenger.");
            ...
            if (result != Result.Success)
            {
                ...
                throw new VulkanException("Failed to create the Vulkan validation debug messenger.", result);
            }
            ...
            void* pUserData)
        {
            ...
            if (pUserData != null)
            {
                try
                {
                    GCHandle callbackHandle = GCHandle.FromIntPtr((nint)pUserData);
                    if (callbackHandle.Target is RendererValidationMessageState validationMessages)
                    {
                        RendererValidationMessageSeverity messageSeverity =
                            (severity & DebugUtilsMessageSeverityFlagsEXT.ErrorBitExt) != 0
                                ...
                                : (severity & DebugUtilsMessageSeverityFlagsEXT.WarningBitExt) != 0
                                    ...
                                    : (severity & DebugUtilsMessageSeverityFlagsEXT.InfoBitExt) != 0
                                        ? RendererValidationMessageSeverity.Information
                                        : RendererValidationMessageSeverity.Verbose;
                        validationMessages.Record(messageSeverity, message);
                    }
                }
                catch (Exception callbackStateException)
                {
                    System.Diagnostics.Debug.WriteLine(
                        $"Vulkan validation callback state update failed: {callbackStateException.Message}");
                }
            }
        ...
        public void ThrowIfValidationFailure()
        {
            _validationMessages.ThrowIfErrorRequested(_validationSettings.FailOnErrorMessage);
        }
