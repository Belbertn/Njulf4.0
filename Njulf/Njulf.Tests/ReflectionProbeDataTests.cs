...
namespace Njulf.Tests
{
    ...
    public class ReflectionProbeDataTests
    {
        ...
        [Test]
        public void BuildProbes_UsesStableLayerResolverAndExcludesUncapturedLayers()
        {
            var settings = new ReflectionSettings { MaxProbes = 2 };
            var uncaptured = new ReflectionProbe { Name = "Uncaptured", Priority = 2 };
            var captured = new ReflectionProbe { Name = "Captured", Priority = 1 };
            var gpu = new GPUReflectionProbe[2];

            int count = ReflectionProbeData.BuildProbes(
                new[] { captured, uncaptured },
                settings,
                gpu,
                probe => ReferenceEquals(probe, uncaptured) ? 11 : 4,
                probe => ReferenceEquals(probe, captured));

            Assert.Multiple(() =>
            {
                Assert.That(count, Is.EqualTo(2));
                Assert.That(gpu[0].CubemapArrayIndex, Is.EqualTo(11));
                Assert.That(gpu[1].CubemapArrayIndex, Is.EqualTo(4));
                Assert.That(gpu[0].Flags & ReflectionProbeData.CapturedRadianceAvailableFlag, Is.EqualTo(0));
                Assert.That(gpu[1].Flags & ReflectionProbeData.CapturedRadianceAvailableFlag, Is.Not.EqualTo(0));
            });
        }
