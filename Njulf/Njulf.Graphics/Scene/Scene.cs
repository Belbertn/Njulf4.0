...
{
    public partial class Scene : IDisposable, IIdentifiedSceneEntity
    {
        ...
        private readonly Dictionary<IDisposable, int> _ownedDisposableReferences = new(ReferenceEqualityComparer.Instance);
        ...
        public Scene()
        {
            ...
            _readOnlyModelInstances = _modelInstances.AsReadOnly();
        ...
        public void Add(IUpdateable updateable)
        {
            ...
            if (_updateables.Contains(updateable))
                throw new InvalidOperationException("The updateable is already attached to this scene.");
            ClaimMembership(updateable);
        ...
        public void Add(FoliagePrototype foliagePrototype)
        {
            ...
            else throw new InvalidOperationException("The foliage prototype is already attached to this scene.");
        }
        ...
        public void Add(FoliagePatch foliagePatch)
        {
            ...
            if (!_foliagePrototypes.Contains(foliagePatch.Prototype)) Add(foliagePatch.Prototype);
        ...
        {
            EnsureMutable();
            EnsureIndividualRemoval(renderObject);
            ...
            ReleaseMembership(renderObject);
            PublishMutation(
                renderObject,
        ...
        {
            EnsureMutable();
            EnsureIndividualRemoval(updateable);
            if (!_updateables.Contains(updateable))
                return;
            if (updateable is IDisposable disposable)
                RemoveDisposableReference(disposable);
            _updateables.Remove(updateable);
            ReleaseMembership(updateable);
        }
        ...
        public void Remove(ReflectionProbe reflectionProbe)
        {
            ...
            {
                ReleaseMembership(reflectionProbe);
                reflectionProbe.Changed -= OnReflectionProbeChanged;
                AdvanceReflectionProbeRevision();
            }
        }
        ...
        {
            EnsureMutable();
            if (_globalIlluminationProbeVolumes.Remove(probeVolume)) ReleaseMembership(probeVolume);
        }
        ...
        public void Remove(VolumetricDensityVolume densityVolume)
        {
            ...
            {
                ReleaseMembership(densityVolume);
                densityVolume.Changed -= OnVolumetricDensityVolumeChanged;
                AdvanceVolumetricDensityRevision();
                PublishMutation(
                    densityVolume,
        ...
        public void Remove(ParticleEffectInstance particleEffect)
        {
            ...
            {
                ReleaseMembership(particleEffect);
                particleEffect.Changed -= OnParticleEffectChanged;
                PublishMutation(
                    particleEffect,
        ...
        public void Remove(StaticInstanceBatch staticInstanceBatch)
        {
            ...
            ReleaseMembership(staticInstanceBatch);
            PublishMutation(
                staticInstanceBatch,
        ...
        public void Remove(FoliagePrototype foliagePrototype)
        {
            ...
            ReleaseMembership(foliagePrototype);
            AdvanceRenderPayloadRevision();
        }
        ...
        public void Remove(FoliagePatch foliagePatch)
        {
            ...
            {
                ReleaseMembership(foliagePatch);
                foliagePatch.Changed -= OnFoliagePatchChanged;
                PublishMutation(
                    foliagePatch,
        ...
        {
            ReleaseClearedMemberships();
            foreach (SceneLight light in _lights) light.Changed -= OnLightChanged;
            if (_lights.Count > 0) LightRevision++;
            _lights.Clear();
            Environment = null;
        ...
        private List<Exception>? DisposeOwnedEntities()
        {
                     ...
                     _ownedDisposableReferences.Keys.ToArray())
            {
                try
                {
                    if (disposable is ModelInstance instance) DisposeInstance(instance);
                    else disposable.Dispose();
                    _ownedDisposableReferences.Remove(disposable);
                    ReleaseAllMemberships(disposable);
                }
        ...
        {
            if (disposable is RenderObject child && _instanceChildren.ContainsKey(child)) return;
        ...
        private void RemoveDisposableReference(IDisposable disposable)
        {
            ...
            if (references <= 1)
            {
                if (!_detaching) disposable.Dispose();
                _ownedDisposableReferences.Remove(disposable);
            }
            else
                _ownedDisposableReferences[disposable] = references - 1;
