...
namespace Njulf.Rendering.Data
{
    public static class ReflectionProbeData
    {
        ...
        {
            return BuildProbes(authoredProbes, settings, destination, null, null);
        }

        /// <summary>
        /// Builds deterministic GPU records. The optional layer and readiness resolvers keep
        /// cubemap-array ownership independent from priority ordering.
        /// </summary>
        public static int BuildProbes(
            IReadOnlyList<ReflectionProbe> authoredProbes,
            ReflectionSettings settings,
            Span<GPUReflectionProbe> destination,
            Func<ReflectionProbe, int>? resolveCubemapArrayIndex,
            Func<ReflectionProbe, bool>? hasCapturedRadiance)
        {
            ...
            {
                ReflectionProbe probe = sortable[i].Probe;
                int layer = resolveCubemapArrayIndex?.Invoke(probe) ?? written;
                destination[written] = BuildGpuProbe(probe, layer, hasCapturedRadiance?.Invoke(probe) ?? true);
                written++;
            }
        ...
        private static GPUReflectionProbe BuildGpuProbe(ReflectionProbe probe, int cubemapArrayIndex, bool hasCapturedRadiance)
        {
            ...
            if (hasCapturedRadiance && cubemapArrayIndex >= 0)
                flags |= CapturedRadianceAvailableFlag;
