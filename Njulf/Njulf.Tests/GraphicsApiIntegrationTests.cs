...
public sealed class GraphicsApiIntegrationTests
{
    ...
    public void CustomResourceDeclarationsRejectDisposedResourcesFeedbackAndInvalidRanges()
    {
        ...
        Assert.That(() => _graphics.AddVulkanPass(new("Invalid.feedback", VulkanPassKind.Graphics, VulkanPassStage.BeforeScene,
            [image, image with { Name = "alias" }]), new EmptyNativePass()), Throws.ArgumentException);
        Assert.That(() => _graphics.AddVulkanPass(new("Invalid.range", VulkanPassKind.Compute, VulkanPassStage.BeforeScene,
            [new VulkanBufferUse("data", Njulf.Rendering.Pipeline.RenderGraphResourceAccess.Read,
                PipelineStageFlags2.ComputeShaderBit, AccessFlags2.ShaderStorageReadBit, buffer, 16, 32)]), new EmptyNativePass()), Throws.TypeOf<ArgumentOutOfRangeException>());
        target.Dispose();
        Assert.That(() => _graphics.AddVulkanPass(new("Invalid.disposed", VulkanPassKind.Graphics, VulkanPassStage.BeforeScene,
    ...
    public unsafe void NativeSlicesDescribeCurrentProductionAllocations()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(bindings.Positions.Buffer.Handle, Is.EqualTo(_buffers.GetBuffer(_meshes.VertexPositionBuffer).Handle));
            Assert.That(bindings.NormalsAndTangents.Buffer.Handle, Is.EqualTo(_buffers.GetBuffer(_meshes.VertexNormalTangentBuffer).Handle));
            Assert.That(bindings.UvsAndColors.Buffer.Handle, Is.EqualTo(_buffers.GetBuffer(_meshes.VertexUvColorBuffer).Handle));
        ...
        foreach (VulkanBufferSlice slice in new[] { bindings.Positions, bindings.NormalsAndTangents, bindings.UvsAndColors, bindings.Indices })
        {
    ...
    public void TypedTextureBindingsKeepEverySharedReferenceAlive()
    {
        ...
        using var second = _graphics.CreateMaterial(MaterialDefinition.Default,
            [new(MaterialTextureSlot.BaseColor, texture), new(MaterialTextureSlot.Emissive, texture)]);
        Assert.That(first.Handle, Is.EqualTo(second.Handle), "Identical typed bindings still use material deduplication.");
        texture.Dispose();
        first.Dispose();
    ...
    public void InvalidTextureCreationAndAssignmentsDoNotConsumeCallerOwnership()
    {
        ...
        using var valid = _graphics.CreateMaterial(MaterialDefinition.Default, [new(MaterialTextureSlot.BaseColor, texture)]);
    }

    private static VulkanMaterial CreateMaterial(VulkanGraphicsDevice graphics) => graphics.CreateMaterial(
        MaterialDefinition.Default with { Name = "Graphics API test", BaseColorFactor = new Vector4(0.3f, 0.6f, 0.9f, 1) });
}
