...
namespace Njulf.Rendering.Core
{
    ...
    public unsafe class SwapchainManager : IDisposable
    {
        ...
        /// captures fail closed when this is false rather than attempting an
        /// invalid image-to-buffer copy.
        /// </summary>
        public bool SupportsTransferSource => _screenshotCapability.TransferSourceSupported;
        /// <summary>
        /// Human-readable explanation of the current renderer screenshot
        /// capability. This is refreshed on every swapchain recreation.
        /// </summary>
        public string ScreenshotCaptureSupportReason => _screenshotCapability.Reason;
        ...
        public SwapchainManager(VulkanContext context, IWindow window)
        {
            ...
            CreateSurface();
            SurfaceCapabilitiesKHR surfaceCapabilities = GetSurfaceCapabilities();
            Extent2D extent = ChooseSwapExtent(_window, surfaceCapabilities);
            CreateSwapchain(default, surfaceCapabilities, extent);
        ...
        private SurfaceCapabilitiesKHR GetSurfaceCapabilities()
        {
            Result result = _context.KhrSurface.GetPhysicalDeviceSurfaceCapabilities(
                _context.PhysicalDevice, _surface, out SurfaceCapabilitiesKHR surfaceCapabilities);
            ...
            Extent2D extent)
        {
            if (extent.Width == 0 || extent.Height == 0)
                throw new InvalidOperationException("A Vulkan swapchain cannot be created with a zero-sized extent.");

            Result result;

            _screenshotCapability = SwapchainScreenshotCapability.Evaluate(surfaceCapabilities.SupportedUsageFlags);
            ...
            var swapchainCreateInfo = new SwapchainCreateInfoKHR
            {
                ...
                ImageExtent = _extent,
                ImageArrayLayers = 1,
                // Some presentation surfaces do not permit TransferSrc on
                // swapchain images. Request it only when advertised; the
                // renderer exposes the capability/reason and fails a capture
                // request explicitly instead of creating an invalid chain.
                ImageUsage = ImageUsageFlags.ColorAttachmentBit |
                    (_screenshotCapability.TransferSourceSupported
        ...
        private static Extent2D ChooseSwapExtent(IWindow window, SurfaceCapabilitiesKHR capabilities)
        {
            ...
            return ChooseSwapExtent(framebufferSize.X, framebufferSize.Y, capabilities);
        }

        internal static Extent2D ChooseSwapExtent(
            ...
            SurfaceCapabilitiesKHR capabilities)
        {
            ...
            // A zero-sized framebuffer is expected while a window is minimized.
            // Preserve that signal so recreation can be deferred; clamping it to
            // MinImageExtent would create a swapchain that does not match the surface.
            if (framebufferWidth <= 0 || framebufferHeight <= 0)
                return default;

            // Clamp to the framebuffer pixel size, not the logical window size.
            uint width = Math.Max(capabilities.MinImageExtent.Width, 
                           Math.Min(capabilities.MaxImageExtent.Width, (uint)framebufferWidth));
            uint height = Math.Max(capabilities.MinImageExtent.Height,
                            Math.Min(capabilities.MaxImageExtent.Height, (uint)framebufferHeight));
        ...
        public bool RecreateSwapchain(Action? waitIdle = null)
        {
            ...
            CreateSwapchain(oldSwapchain, surfaceCapabilities, extent);
            CreateDepthResources();
