...
namespace Njulf.Rendering.Resources
{
    ...
    internal sealed unsafe class SimpleDdgiSampledAtlas : IDisposable
    {
        ...
        private int[] _orderedUpdateIndices = Array.Empty<int>();
            ...
            ReadOnlySpan<GPUSimpleDdgiProbeUpdate> updates)
        {
            ...
            for (int groupIndex = 0; groupIndex < _groups.Length; groupIndex++)
            {
                ...
                if (updateCount == 0)
                    continue;
                ReadOnlySpan<int> updateIndices = new(
                    _orderedUpdateIndices,
        ...
        private int BuildUpdatedRegions(
            ReadOnlySpan<GPUSimpleDdgiProbeUpdate> updates,
            ReadOnlySpan<int> updateIndices,
            int groupIndex,
            int texelsPerProbe)
        {
            ulong bytesPerProbe = BytesPerProbe(texelsPerProbe);
            int regionCount = 0;
            int firstProbe = checked(groupIndex * _layersPerTexture);
            for (int orderedIndex = 0; orderedIndex < updateIndices.Length; orderedIndex++)
            {
                int updateIndex = updateIndices[orderedIndex];
                uint probeIndex = updates[updateIndex].ProbeIndex;
                int layerIndex = checked((int)probeIndex - firstProbe);
                if ((uint)layerIndex >= (uint)_groups[groupIndex].LayerCount)
                {
                    continue;
                }

                _regionScratch[regionCount++] = new BufferImageCopy
                {
                    BufferOffset = checked((ulong)probeIndex * bytesPerProbe),
                    BufferRowLength = 0,
                    BufferImageHeight = 0,
                    ImageSubresource = new ImageSubresourceLayers
                    {
                        AspectMask = ImageAspectFlags.ColorBit,
                        MipLevel = 0,
                        BaseArrayLayer = checked((uint)layerIndex),
                        LayerCount = 1
                    },
                    ImageOffset = new Offset3D { X = 0, Y = 0, Z = 0 },
                    ImageExtent = new Extent3D
        ...
        private int BuildGroupedUpdateIndexRanges(ReadOnlySpan<GPUSimpleDdgiProbeUpdate> updates)
        {
            ...
            for (int updateIndex = 0; updateIndex < updates.Length; updateIndex++)
            {
                ...
                _orderedUpdateIndices[_groupRegionCursors[groupIndex]++] = updateIndex;
            }
        ...
        {
            if (_orderedUpdateIndices.Length < count)
                _orderedUpdateIndices = new int[count];
        }
