    ...
    public sealed class DdgiGatherTileManagerTests
    {
        ...
        public void BuildTiles_SelectsProjectedLocalVolumeAndClipmapPair()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(result.Header.Flags & DdgiGatherTileManager.HeaderSimpleDdgiFlag, Is.Zero);
        ...
        [Test]
        public void BuildSimpleTiles_PreservesPriorityOrderAndCapsEachTileAtTwoCandidates()
        {
            GPUSimpleDdgiVolume[] volumes =
            {
                CreateSimpleVolume(new Vector3(-1.0f, -1.0f, -0.5f), new Vector3(1.0f, 1.0f, 0.5f)),
                CreateSimpleVolume(new Vector3(-1.0f, -1.0f, -0.5f), new Vector3(1.0f, 1.0f, 0.5f))
            };
            var tiles = new GPUDdgiGatherTile[4];

            DdgiGatherTileManager.BuildResult result = DdgiGatherTileManager.BuildSimpleTiles(
                volumes,
                Matrix4x4.Identity,
                screenWidth: 32,
                screenHeight: 32,
            ...
            {
                Assert.That(result.TileCount, Is.EqualTo(4));
                Assert.That(result.SelectedLocalTileCount, Is.EqualTo(4));
                Assert.That(result.SelectedClipmapTileCount, Is.EqualTo(4));
                Assert.That(result.FallbackTileCount, Is.EqualTo(0));
                Assert.That(result.Header.Flags & DdgiGatherTileManager.HeaderEnabledFlag, Is.Not.Zero);
                Assert.That(result.Header.Flags & DdgiGatherTileManager.HeaderSimpleDdgiFlag, Is.Not.Zero);
                Assert.That(tiles, Has.All.Matches<GPUDdgiGatherTile>(tile =>
                    tile.LocalVolumeIndex == 0u &&
                    tile.PrimaryClipmapVolumeIndex == 1u &&
                    tile.SecondaryClipmapVolumeIndex == DdgiGatherTileManager.InvalidVolumeIndex &&
                    (tile.Flags & DdgiGatherTileManager.TileLocalVolumeValidFlag) != 0u &&
                    (tile.Flags & DdgiGatherTileManager.TilePrimaryClipmapValidFlag) != 0u &&
                    (tile.Flags & DdgiGatherTileManager.TileSecondaryClipmapValidFlag) != 0u &&
                    (tile.Flags & DdgiGatherTileManager.TileFallbackFlag) == 0u &&
        ...
        [Test]
        public void BuildSimpleTiles_RecordsThirdProjectedCandidateWithoutDiscardingThePriorityFastPath()
        {
            GPUSimpleDdgiVolume[] volumes =
            {
                CreateSimpleVolume(new Vector3(-1.0f, -1.0f, -0.5f), new Vector3(1.0f, 1.0f, 0.5f)),
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(tiles, Has.All.Matches<GPUDdgiGatherTile>(tile =>
                    ...
                    tile.PrimaryClipmapVolumeIndex == 1u &&
                    (tile.Flags & DdgiGatherTileManager.TileFallbackFlag) == 0u &&
                    (tile.Flags & DdgiGatherTileManager.TileSimpleCandidateOverflowFlag) != 0u &&
                    tile.BlendWeights.W == 0.0f));
            });
        }

        [Test]
        public void BuildSimpleTiles_MarksOffscreenVolumesForSafeFallback()
        {
            GPUSimpleDdgiVolume[] volumes =
            {
                CreateSimpleVolume(new Vector3(8.0f, 8.0f, -0.5f), new Vector3(9.0f, 9.0f, 0.5f))
            };
            var tiles = new GPUDdgiGatherTile[4];
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(tiles, Has.All.Matches<GPUDdgiGatherTile>(tile =>
                    tile.LocalVolumeIndex == DdgiGatherTileManager.InvalidVolumeIndex &&
                    tile.PrimaryClipmapVolumeIndex == DdgiGatherTileManager.InvalidVolumeIndex &&
                    (tile.Flags & DdgiGatherTileManager.TileFallbackFlag) != 0u &&
                    tile.BlendWeights.W == 1.0f));
            });
        }
        ...
        [Test]
        public void BuildSimpleTiles_RejectsInvalidBoundsWithoutEnablingCandidateTable()
        {
            GPUSimpleDdgiVolume[] volumes =
            {
                CreateSimpleVolume(
                    new Vector3(float.NaN, -1.0f, -0.5f),
            ...
            Assert.Multiple(() =>
            {
                Assert.That(result.Header.Flags, Is.EqualTo(0u));
                Assert.That(result.SelectedLocalTileCount, Is.EqualTo(0));
                Assert.That(result.SelectedClipmapTileCount, Is.EqualTo(0));
                Assert.That(result.FallbackTileCount, Is.EqualTo(0));
                Assert.That(tiles[0].LocalVolumeIndex, Is.EqualTo(DdgiGatherTileManager.InvalidVolumeIndex));
                Assert.That(tiles[0].PrimaryClipmapVolumeIndex, Is.EqualTo(DdgiGatherTileManager.InvalidVolumeIndex));
                Assert.That(tiles[0].Flags, Is.EqualTo(0u));
            });
        }
        ...
        private static GPUSimpleDdgiVolume CreateSimpleVolume(Vector3 worldMin, Vector3 worldMax)
        {
            return new GPUSimpleDdgiVolume
            {
                OriginAndSpacing = new Vector4(worldMin.X, worldMin.Y, worldMin.Z, 1.0f),
                GridCountsAndFirstProbe = new Vector4(2.0f, 2.0f, 2.0f, 0.0f),
                WorldMinAndEdgeFade = new Vector4(worldMin.X, worldMin.Y, worldMin.Z, 1.0f),
                WorldMaxAndKind = new Vector4(worldMax.X, worldMax.Y, worldMax.Z, 1.0f)
            };
        }
    }
}
