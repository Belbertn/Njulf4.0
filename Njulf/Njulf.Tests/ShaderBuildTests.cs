...
public sealed class ShaderBuildTests
{
    ...
    public void GlobalSdfShaders_UseToroidalClipmapBrickAddressing()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(update, Does.Contain("const uint GLOBAL_SDF_THIN_FEATURE_FIRST_CASCADE = 2u;"));
            Assert.That(update, Does.Contain("const float GLOBAL_SDF_THIN_FEATURE_CENTER_BAND_VOXELS = 0.5;"));
            ...
            Assert.That(update, Does.Contain("bool AllFullVoxelNeighborsPositive(vec3 worldPosition, uint candidateCount, float voxelSize)"));
            Assert.That(update, Does.Contain("bool HasSurfaceCrossingWithinVoxel(vec3 worldPosition, uint candidateCount, float voxelSize)"));
            Assert.That(update, Does.Contain("float PreserveThinFeatureNegativeCore(vec3 worldPosition, float centerDistance, uint candidateCount)"));
            Assert.That(update, Does.Contain("pc.Push.CascadeIndex < GLOBAL_SDF_THIN_FEATURE_FIRST_CASCADE"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition + dx, candidateCount) > 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition - dx, candidateCount) > 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition + dy, candidateCount) > 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition - dy, candidateCount) > 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition + dz, candidateCount) > 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition - dz, candidateCount) > 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition + dx, candidateCount) < 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition - dx, candidateCount) < 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition + dy, candidateCount) < 0.0"));
            ...
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition + dz, candidateCount) < 0.0"));
            Assert.That(update, Does.Contain("ComposeGlobalSdfDistance(worldPosition - dz, candidateCount) < 0.0"));
            Assert.That(update, Does.Contain("return min(centerDistance, -voxelSize * GLOBAL_SDF_THIN_FEATURE_NEGATIVE_CORE_VOXELS);"));
            Assert.That(update, Does.Contain("distanceMeters = ComposeGlobalSdfDistance(worldPosition, candidateCount);"));
            Assert.That(update, Does.Contain("distanceMeters = PreserveThinFeatureNegativeCore(worldPosition, distanceMeters, candidateCount);"));
