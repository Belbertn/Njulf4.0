...
public sealed class ProductionRenderPipelineDeclarationTests
{
    ...
    [Test]
    public void ForwardPlus_DeclaresTheStrictDepthPrePassReadContract()
    {
        ProductionRenderPipelineDeclaration declaration = ProductionRenderPipelineDeclaration.Instance;
        RenderGraphResourceUsage usage = declaration.PassResourceDeclarations
    ...
    [Test]
    public void DepthPrePass_PrecedesAllForwardDepthConsumersAndTiledLightCullingReadsIt()
    {
        ProductionRenderPipelineDeclaration declaration = ProductionRenderPipelineDeclaration.Instance;
        string[] passOrder = declaration.PassOrder.ToArray();
        RenderGraphResourceUsage depthPrePassUsage = declaration.PassResourceDeclarations
            .Single(pass => pass.PassName == "DepthPrePass")
            .Usages
            .Single(usage => usage.Resource == RenderGraphResourceId.SceneDepth);
        RenderGraphResourceUsage tiledLightUsage = declaration.PassResourceDeclarations
            .Single(pass => pass.PassName == "TiledLightCullingPass")
            .Usages
            .Single(usage => usage.Resource == RenderGraphResourceId.SceneDepth);
        string[] depthConsumers =
        [
            "MotionVectorPass",
            "HiZBuildPass",
            "SceneSurfacePass",
        ...
        Assert.Multiple(() =>
        {
            Assert.That(Array.IndexOf(passOrder, "DepthPrePass"), Is.LessThan(Array.IndexOf(passOrder, "TiledLightCullingPass")));
            Assert.That(Array.IndexOf(passOrder, "TiledLightCullingPass"), Is.LessThan(Array.IndexOf(passOrder, "ForwardPlusPass")));
            Assert.That(depthPrePassUsage.Access, Is.EqualTo(RenderGraphResourceAccess.Write));
            Assert.That(depthPrePassUsage.ImageLayout, Is.EqualTo(ImageLayout.DepthStencilAttachmentOptimal));
            Assert.That(tiledLightUsage.Access, Is.EqualTo(RenderGraphResourceAccess.Read));
            Assert.That(tiledLightUsage.ImageLayout, Is.EqualTo(ImageLayout.DepthStencilReadOnlyOptimal));
            foreach (string passName in depthConsumers)
            {
                RenderGraphResourceUsage usage = declaration.PassResourceDeclarations
                    .Single(pass => pass.PassName == passName)
                    .Usages
                ...
                Assert.That(Array.IndexOf(passOrder, "DepthPrePass"), Is.LessThan(Array.IndexOf(passOrder, passName)), passName);
