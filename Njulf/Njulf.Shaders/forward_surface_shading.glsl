// Shared forward surface and optical hit shading. Stage entry points remain in their own files.
...
#ifdef NJULF_OPTICAL_COMPUTE
#include "optical_forward_frame.glsl"
#else
    ...
    out vec3 debugColor)
{
    ...
    bool deferOptical = OpticalDeferredShading();
    if (deferOptical && !geometricHit && screenGeometricEnabled)
    {
        opticalTraceFlags |= 1u;
        opticalReflectionDirection = reflectionDirection;
        opticalSchedulingRoughness = schedulingRoughness;
        opticalReflectionObserved = 0.0;
    }
    if (!geometricHit && screenGeometricEnabled && !deferOptical)
    {
