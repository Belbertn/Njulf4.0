...
public sealed class SampleDdgiBenchmarkSuiteTests
{
    ...
    public void ProductionGate_PassesForDdgiHighReportWithoutSsgiOrSteadyChurn()
    {
        SampleBenchmarkReport report = CreateGateReport(RendererDiagnostics.Empty with
        {
            ...
            AccelerationStructureTopLevelInstanceCount = 128,
            AccelerationStructureStaticInstanceCandidateCount = 128,
            AccelerationStructureStaticInstanceResidentCount = 128,
    ...
    public void ProductionGate_RejectsIncompleteSponzaRaySceneAndInactiveRequestedFarField()
    {
        SampleBenchmarkReport report = CreateGateReport(RendererDiagnostics.Empty with
        {
            ...
            AccelerationStructureTopLevelInstanceCount = 210,
            AccelerationStructureBlasBudgetRejectedCount = 236,
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(failures, Does.Contain("ddgi-ray-query-scene-complete"));
            Assert.That(failures, Does.Contain("requested-paged-far-field-active"));
        });
    }
    ...
    public void ProductionGate_FailsWhenPhase10MetricsCollapse()
    {
        SampleBenchmarkReport report = CreateGateReport(RendererDiagnostics.Empty with
        {
            ...
            DdgiAverageDataConfidenceEstimate = 0.0f,
            DdgiAverageVisibilityConfidenceEstimate = 0.0f,
            DdgiAverageLeakAttenuationEstimate = 0.0f,
            DdgiAverageEffectiveContributionEstimate = 0.0f,
            DdgiAverageOwnershipConsumedEstimate = 0.0f,
