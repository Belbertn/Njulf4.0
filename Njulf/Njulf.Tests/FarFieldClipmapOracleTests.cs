    ...
    public sealed class FarFieldClipmapOracleTests
    {
        ...
        public void FarFieldTierAdmission_ReservesStaticInputAndKeepsBothCacheModesWithinTheHardCap()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(FarFieldClipmapManager.CalculateLegacyCacheBytes(legacyResolution), Is.LessThanOrEqualTo(pageCacheBudget));
                Assert.That(pageCapacity, Is.GreaterThan(0));
                Assert.That(FarFieldClipmapManager.CalculatePagedCacheBytes(pageResolution, pageCapacity), Is.LessThanOrEqualTo(pageCacheBudget));
            });
        }
