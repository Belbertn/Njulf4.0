    ...
    public sealed unsafe class SkyboxPipeline : IDisposable
    {
        ...
        private void ValidatePushConstantRange(uint requiredSize)
        {
            ...
            if (requiredSize > properties.Limits.MaxPushConstantsSize)
                throw new VulkanException($"Skybox pass requires {requiredSize} bytes of push constants but GPU supports {properties.Limits.MaxPushConstantsSize}.");
        }
        ...
        private void CreatePipelineCache()
        {
            ...
            if (result != Result.Success)
                throw new VulkanException("Failed to create skybox pipeline cache", result);
            _context.SetDebugName(_pipelineCache.Handle, ObjectType.PipelineCache, "Skybox Pipeline Cache");
        }
        ...
        private void CreatePipelineLayout()
        {
            ...
            var pushConstantRange = new PushConstantRange
            {
                ...
                Offset = 0,
                Size = (uint)Marshal.SizeOf<GPUSkyboxPushConstants>()
            };
            ...
            if (result != Result.Success)
                throw new VulkanException("Failed to create skybox pipeline layout", result);
            _context.SetDebugName(_layout.Handle, ObjectType.PipelineLayout, "Skybox Pipeline Layout");
        }
        ...
        private void CreatePipeline(Format colorFormat, Format depthFormat)
        {
            ...
            try
            {
                ...
                fragmentModule = ShaderModuleLoader.Load(_context, "skybox.frag.spv");
                _context.SetDebugName(vertexModule.Handle, ObjectType.ShaderModule, "composite.vert.spv");
                _context.SetDebugName(fragmentModule.Handle, ObjectType.ShaderModule, "skybox.frag.spv");

                _pipeline = CreateGraphicsPipeline(vertexModule, fragmentModule, colorFormat, depthFormat);
                _context.SetDebugName(_pipeline.Handle, ObjectType.Pipeline, "Skybox Pipeline");
            }
            finally
            {
        ...
        private VkPipeline CreateGraphicsPipeline(ShaderModule vertexModule, ShaderModule fragmentModule, Format colorFormat, Format depthFormat)
        {
