...
namespace Njulf.Tests
{
    ...
    {
        private string? _previousSourceFallback;

        [SetUp]
        public void EnableExplicitSourceImportForImporterIntegrationTests()
        {
            _previousSourceFallback = Environment.GetEnvironmentVariable("NJULF_ALLOW_SOURCE_ASSET_RUNTIME_LOAD");
            Environment.SetEnvironmentVariable("NJULF_ALLOW_SOURCE_ASSET_RUNTIME_LOAD", "true");
        }

        [TearDown]
        public void RestoreSourceImportPolicy() =>
            Environment.SetEnvironmentVariable("NJULF_ALLOW_SOURCE_ASSET_RUNTIME_LOAD", _previousSourceFallback);
        ...
        public void LoadProcessedMeshAsset_BuildsProcessedAssetAndCachesIt()
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(first.SubMeshes[0].Meshlets, Has.Length.EqualTo(3));
                Assert.That(first.SubMeshes[0].LodRanges, Has.Count.EqualTo(3));
