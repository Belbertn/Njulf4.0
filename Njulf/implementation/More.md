...
## REASSESSMENT (after further user pushes — circumstances changed)

The user's continued iteration has resolved most of what motivated the full rewrite. Current state at `c8a791e`:
- **Surface cache fallback = 0.00%** (was 2–3%); `lookups==hits`. Card-projection debug (images 4/5) shows full card coverage → cache is healthy. Phase 4 is now largely MOOT.
- **SDF trace**: `avgSteps` ~5–7, `stepExhausted` ~10–70/frame (~0.2–0.9%, was 200+). Lit scene (image 1) is clean; only the raw SDF debug view shows minor silhouette hatching (cosmetic).
- Backend heatmap (image 3): SDF backend serving with decent confidence; dark speckle = low-confidence far/occluded pockets (minor).
- **The ONLY thing tripping "Degraded" is the `stepExhausted > 0` hair-trigger** (`SampleDiagnosticsReporter.cs:549`).

**Honest cost/benefit:** the full JCGT DDA+cubic rewrite is now *marginal polish*, not a fix for a live defect. Its remaining value: drive stepExhausted→0 properly, crisper SDF silhouettes (debug-only), slightly tighter hit precision (cache already 100%, ~no gain). Recommended default is to **descope** to the two cheap targeted fixes below and hold the full rewrite unless the user wants the debug-view crispness / future-proofing.

### APPROVED SCOPE: targeted fixes only

**Fix 1 — Triage gate honesty (`NjulfHelloGame/SampleDiagnosticsReporter.cs`, `ClassifySdfState`).**
The gate flags "Degraded" on a single exhausted ray. There are **two** conditions to change (both currently keyed on `stepExhausted`):
- Line 549: `bool degraded = d.DdgiSdfStepExhaustedCount > 0u || highInsideStarts || FallbackPercent > 10.0f;`
- Line 561: `if (d.GlobalSdfDirtyBrickBacklog <= 0 && d.DdgiSdfStepExhaustedCount == 0) return "SteadyState";` (else falls through to `return "Degraded"`).
Introduce one shared predicate and use it in both:
```csharp
uint stepExhaustBudget = Math.Max(4u, traceCount / 100u);           // ~1% of traces
bool stepExhaustionElevated = d.DdgiSdfStepExhaustedCount > stepExhaustBudget;
// :549 use stepExhaustionElevated instead of ">0u"
// :561 use `!stepExhaustionElevated` instead of `== 0`
```
At current rates (~10–70 of ~8000 traces ≈ 0.1–0.9%) this yields `SteadyState`. No test pins `ClassifySdfState` output strings (verified: `DdgiDiagnosticsFormattingTests` doesn't assert these); if a triage-state test surfaces at build, update its expectation.

**Fix 2 — Reduce grazing step exhaustion (`ddgi_update_shared.glsl`, `TraceProbeRay:2298`).**
Raise the DDGI SDF step cap `96u` → `160u`. This is correctness-safe (pure budget), cost falls only on the ~0.5% grazing rays that currently exhaust (`avgSteps` for the other 99.5% is ~5–7, unaffected). *Do not* add a >1 overstep multiplier to the conservative march — it risks missing thin surfaces (reintroduces the class of error we just eliminated). If 160 doesn't materially cut `stepExhausted`, stop there and rely on Fix 1 — the residual is genuinely-hard grazing geometry, not a defect.

**Out of scope (per user):** the full DDA+cubic rewrite, surface-cache Phase 4 (cache is at 0% fallback), hardware-trilinear Phase 3. The full rewrite spec is retained below as an optional appendix only.

### Verification (targeted fixes)
1. `dotnet build` shaders + `dotnet test` (should be green with no/minimal test edits — no shader-pin churn since `global_sdf.glsl` is untouched).
2. Run the sample, same viewpoints: `SDF TRIAGE: state=` should read **SteadyState** in steady state; `stepExhausted` should drop (target near-0, acceptable if a handful remain); `avgSteps` and `gpuUs sdf` should not materially regress; surface-cache fallback stays 0%; lit scene unchanged.
3. Commit both fixes on `claude/sdf-surface-cache-quality-qhbyll`, push `git push -u origin`.

---

## OPTIONAL APPENDIX (not in approved scope) — full DDA+cubic rewrite
