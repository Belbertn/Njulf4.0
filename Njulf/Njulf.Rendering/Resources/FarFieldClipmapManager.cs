...
namespace Njulf.Rendering.Resources
{
    public sealed class FarFieldClipmapManager : IDisposable
    {
        ...
        public int Resolution => _pagedMode ? _pageResolution : _settings.GlobalIllumination.FarFieldClipmapResolution;
        public bool BakePending => _bakePending;
        public bool PagedMode => _pagedMode;
        public int PagePoolCapacity => _pagePoolCapacity;
        public int ResidentPageCount => _pageCache.ResidentCount;
        public int PendingPageCount => _pageCache.PendingCount;
        public int PageEvictionCount => _pageCache.EvictionCount;
        ...
        private static int ResolvePagedPageResolution(GlobalIlluminationSettings gi)
        {
            int resolution = gi.FarFieldPageResolution;
            while (resolution > 16 && CalculatePagedCacheBytes(resolution, 1) > gi.FarFieldMemoryBudgetBytes)
                resolution = Math.Max(16, resolution / 2);
            return resolution;
        }

        private static int ResolvePagedPageCapacity(GlobalIlluminationSettings gi, int resolution)
        {
            int capacity = gi.FarFieldResidentPageBudget;
            while (capacity > 1 && CalculatePagedCacheBytes(resolution, capacity) > gi.FarFieldMemoryBudgetBytes)
                capacity--;
            return capacity;
        }

        private static ulong CalculatePagedCacheBytes(int resolution, int capacity)
        {
            ulong pageVoxelCount = GetPageVoxelCount(resolution);
