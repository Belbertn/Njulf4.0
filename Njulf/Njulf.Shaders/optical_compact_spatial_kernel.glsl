void OpticalCompactSpatialPixel(ivec2 p) {if(!OcInside(p))return;uint b=pc.Current;
 for(uint layer=0u;layer<2u;layer++) {
  ...
  float geometryWeights[9]; i=0u;
  vec3 normal=OpticalNormal(OpticalWord(b,r+3u)); float power=mix(128.0,16.0,OpticalFloat(b,r+7u));
  for(int y=-1;y<=1;y++)for(int x=-1;x<=1;x++) {
   uint s=neighbors[i]; geometryWeights[i++]=s==OPTICAL_INVALID?0.0:(x==0?2.0:1.0)*(y==0?2.0:1.0)*pow(max(dot(normal,OpticalNormal(OpticalWord(b,s+3u))),0),power);
  }
