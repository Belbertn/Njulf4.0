...
public sealed class ShaderBuildTests
{
    ...
    public void GlobalSdfShaders_UseToroidalClipmapBrickAddressing()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(sampling, Does.Contain("#define GLOBAL_SDF_TRACE_ANALYTIC 1"));
            Assert.That(sampling, Does.Contain("const float GLOBAL_SDF_TRACE_ANALYTIC_BAND_VOXELS = 1.5;"));
            Assert.That(sampling, Does.Contain("const uint GLOBAL_SDF_TRACE_ANALYTIC_CASCADE_INDEX = 2u;"));
            Assert.That(sampling, Does.Contain("const uint GLOBAL_SDF_TRACE_ANALYTIC_MAX_CELLS = 8u;"));
            Assert.That(sampling, Does.Contain("const uint GLOBAL_SDF_TRACE_ANALYTIC_NEWTON_ITERATIONS = 2u;"));
            ...
            Assert.That(sampling, Does.Contain("float EvaluateGlobalSdfTrilinearCell("));
            Assert.That(sampling, Does.Contain("bool TryTraceGlobalSdfAnalyticCells("));
            Assert.That(sampling, Does.Contain("GLOBAL_SDF_TRACE_ANALYTIC_NEWTON_ITERATIONS"));
            Assert.That(sampling, Does.Contain("if (cascadeIndex == GLOBAL_SDF_TRACE_ANALYTIC_CASCADE_INDEX"));
