...
public sealed class ShaderBuildTests
{
    ...
    public void SdfCascadeValidationScene_SpansMultipleSdfCascadesAndSurfaceCachePath()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(validation, Does.Contain("gi.SdfBackendFirstCascade = 2;"));
    ...
    public void DdgiUpdateShader_UsesFullSphereFibonacciSamplingAndGatheredVisibility()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(shader, Does.Contain("if (!forceAnalyticFallback && TrySampleDdgiSurfaceCacheRadiance(hitPosition, surfaceNormal, surfaceAlbedo, surfaceCacheHitErrorMeters, cacheRadiance))"));
            Assert.That(shader, Does.Contain("if (!forceAnalyticFallback && TrySampleDdgiSurfaceCacheRadiance(hitPosition, surfaceNormal, surfaceAlbedo, 0.01, cacheRadiance))"));
    ...
    public void ForwardShader_ExposesDdgiCoverageAndCascadeDebugViews()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(shader, Does.Contain("uint physicalIndex = GlobalSdfPhysicalBrickIndex(hitPosition, cascade);"));
    ...
    public void GlobalSdfShaders_UseToroidalClipmapBrickAddressing()
    {
        ...
        string volumeTexture = ReadRepoText("Njulf.Rendering", "Resources", "VolumeTexture.cs");
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(common, Does.Contain("uint BrickStateWordOffset;"));
            Assert.That(common, Does.Contain("uint BrickStateWordCount;"));
            ...
            Assert.That(common, Does.Contain("vec4 LocalToWorldAxisScale;"));
            Assert.That(common, Does.Contain("const int SIZEOF_GPU_MESH_SDF = 176;"));
            Assert.That(common, Does.Contain("const int OFFSET_GPU_MESH_SDF_LOCAL_TO_WORLD_AXIS_SCALE = 112;"));
            ...
            Assert.That(readMeshSdf, Does.Contain("result.LocalToWorldAxisScale = ReadStorageVec4(pc.Push.MeshSdfBufferIndex, baseWord + 28u);"));
            ...
            Assert.That(update, Does.Contain("const float GLOBAL_SDF_VOXEL_SAMPLE_GATE_VOXELS = 0.95;"));
            Assert.That(update, Does.Contain("const float GLOBAL_SDF_VOXEL_AXIS_SAMPLE_OFFSET = 0.45;"));
            Assert.That(update, Does.Contain("const float GLOBAL_SDF_VOXEL_CORNER_SAMPLE_OFFSET = 0.35;"));
            Assert.That(update, Does.Contain("float ComposeGlobalSdfDistance(vec3 worldPosition, uint candidateCount)"));
            Assert.That(update, Does.Contain("void AccumulateConservativeVoxelSample("));
            Assert.That(update, Does.Contain("float ComposeConservativeGlobalSdfVoxelDistance(vec3 worldPosition, uint candidateCount, float voxelSize)"));
            Assert.That(update, Does.Contain("if (bestNegativeAbsDistance < 1.0e19)"));
            Assert.That(update, Does.Contain("return bestNegativeDistance;"));
            Assert.That(update, Does.Contain("return bestPositiveDistance < 1.0e19 ? bestPositiveDistance : centerDistance;"));
            Assert.That(update, Does.Contain("distanceMeters = ComposeConservativeGlobalSdfVoxelDistance("));
            Assert.That(update, Does.Contain("abs(centerDistance) > voxelSize * GLOBAL_SDF_VOXEL_SAMPLE_GATE_VOXELS"));
            Assert.That(update, Does.Contain("Production rule: negative only comes from a real inside sample within this voxel cell."));
            ...
            Assert.That(sampling, Does.Contain("uint GlobalSdfPhysicalBrickIndex(vec3 worldPosition, GPUGlobalSdfCascade cascade)"));
            Assert.That(sampling, Does.Contain("GLOBAL_SDF_DIAGNOSTIC_SAMPLE(worldPosition, cascade, cascadeIndex);"));
            Assert.That(sampling, Does.Contain("float DecodeGlobalSdfDistance(float normalizedDistance, float voxelSize)"));
            ...
            Assert.That(sampling, Does.Contain("GlobalSdfCellCorners FetchGlobalSdfCellCorners"));
            Assert.That(sampling, Does.Contain("float EvaluateGlobalSdfTrilinear(GlobalSdfCellCorners c, vec3 localP)"));
            Assert.That(sampling, Does.Contain("vec3 AnalyticTrilinearGradient(GlobalSdfCellCorners c, vec3 localP, float voxelSize)"));
            Assert.That(sampling, Does.Contain("vec4 GlobalSdfTrilinearCubicCoefficients(GlobalSdfCellCorners c, vec3 aLocal, vec3 bLocal)"));
            ...
            Assert.That(sampling, Does.Contain("if (abs(sampleValue.DistanceMeters) > voxelSize * GLOBAL_SDF_TRACE_NEAR_BAND_VOXELS)"));
            Assert.That(sampling, Does.Contain("float coarseStep = max(sampleValue.DistanceMeters - voxelSize * GLOBAL_SDF_TRACE_NEAR_BAND_VOXELS, minStep);"));
            Assert.That(sampling, Does.Contain("GlobalSdfCellCorners corners = FetchGlobalSdfCellCorners(logicalCell, cascade);"));
            Assert.That(sampling, Does.Contain("if (ddaCells >= GLOBAL_SDF_DDA_MAX_CELLS)"));
            ...
            Assert.That(ddgi, Does.Contain("const uint DDGI_SURFACE_CACHE_REJECT_GRID_MISS_COUNTER = DDGI_SURFACE_CACHE_COUNTER_BASE + 10u;"));
            ...
            Assert.That(ddgi, Does.Contain("const uint DDGI_GLOBAL_SDF_DIRTY_PHYSICAL_BRICK_SAMPLE_COUNTER = DDGI_SURFACE_CACHE_COUNTER_BASE + 26u;"));
            Assert.That(ddgi, Does.Contain("const uint DDGI_SURFACE_CACHE_REJECT_MOVEMENT_COUNTER_BASE = DDGI_SURFACE_CACHE_COUNTER_BASE + 27u;"));
            Assert.That(ddgi, Does.Contain("const uint DDGI_SURFACE_CACHE_REJECT_MOVEMENT_COUNTER_STRIDE = 5u;"));
            Assert.That(ddgi, Does.Contain("const uint DDGI_CAMERA_MOVEMENT_CLASS_COUNT = 7u;"));
            Assert.That(ddgi, Does.Contain("const uint DDGI_SURFACE_CACHE_REJECT_REASON_GRID_MISS = 0u;"));
            Assert.That(ddgi, Does.Contain("const uint DDGI_SURFACE_CACHE_REJECT_REASON_DEPTH_UV = 1u;"));
            Assert.That(ddgi, Does.Contain("const uint DDGI_SURFACE_CACHE_REJECT_REASON_NORMAL_AXIS = 2u;"));
            ...
            Assert.That(ddgi, Does.Contain("void AddDdgiGlobalSdfSampleDiagnostics(vec3 worldPosition, GPUGlobalSdfCascade cascade, uint cascadeIndex)"));
            Assert.That(ddgi, Does.Contain("ReadStorageUint(pc.GlobalSdfCascadeBufferIndex, cascade.BrickStateWordOffset + wordIndex)"));
            Assert.That(ddgi, Does.Contain("AddRendererDiagnostic(pc.CurrentFrameIndex, DDGI_GLOBAL_SDF_DIRTY_PHYSICAL_BRICK_SAMPLE_COUNTER, 1u);"));
            Assert.That(ddgi, Does.Contain("bool depthUvDominant = depthUvRejects >= normalRejects"));
            Assert.That(ddgi, Does.Contain("AddDdgiSurfaceCacheRejectDiagnostic("));
    ...
    public void SurfaceCacheShaders_UseWorkBufferGridAndCacheFirstHitShading()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(ddgi, Does.Contain("SampleDdgiSurfaceCacheCandidate"));
