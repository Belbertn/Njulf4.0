...
public sealed class SimpleDdgiSampledAtlasTests
{
    ...
    public void ProbeCapacityProvisioning_BoundsReallocationChurnAndDescriptorRange()
    {
        Assert.Multiple(() =>
        {
            ...
            Assert.That(BindlessIndex.OpaqueSceneColorSnapshotTextureB + 1,
                Is.EqualTo(BindlessIndex.GtaoReferenceNormalTexture));
            Assert.That(BindlessIndex.GtaoReferenceNormalTexture + 1,
    ...
    {
        const ulong bytesPerProbe = 10UL * 10UL * 8UL + 18UL * 18UL * 4UL;
