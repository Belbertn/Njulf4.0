...
public sealed class ShaderBuildTests
{
    ...
    [Test]
    public void MotionVectorPass_ReportsItsEarlyOutToRenderGraphScheduling()
    {
        string source = ReadRepoText("Njulf.Rendering", "Pipeline", "MotionVectorPass.cs");

        Assert.That(
            source,
            Does.Contain("public override bool ShouldExecute(int frameIndex, SceneRenderingData sceneData)")
                .And.Contain("return NeedsMotionVectors();"));
    }
    ...
    [Test]
    public void ForwardMaterialPaths_UseCorrectEmissiveTbnAndProbeArrayContracts()
    {
        ...
        string materialManager = ReadRepoText("Njulf.Rendering", "Resources", "MaterialManager.cs");
        string modelUpload = ReadRepoText("Njulf.Rendering", "Resources", "ModelRenderUploadService.cs");
        string probeManager = ReadRepoText("Njulf.Rendering", "Resources", "ReflectionProbeManager.cs");
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(mesh, Does.Contain("objectWordOffset + 16u"));
            Assert.That(mesh, Does.Contain("TransformRowMajorVector(vertex.Tangent.xyz, instanceBufferIndex, objectWordOffset)"));
            Assert.That(mesh, Does.Contain("ReadRowMajorLinearDeterminant"));
            Assert.That(forward, Does.Contain("BindlessCubeArrayTextures"));
            Assert.That(forward, Does.Contain("REFLECTION_PROBE_CAPTURED_RADIANCE_AVAILABLE_FLAG"));
            Assert.That(probeManager, Does.Contain("Dictionary<Guid, int> _layersByProbeId"));
            Assert.That(probeManager, Does.Contain("SortedSet<int> _freeLayers"));
            Assert.That(probeManager, Does.Contain("public void PublishCapture"));
        });
    }
    ...
    public void CornellValidationScene_UsesCameraRelativeClipmapAndSolidRoomGeometry()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(builder, Does.Contain("EmissionGiParticipation = GiParticipationOverride.Disabled"));
    ...
    [Test]
    public void DdgiUpdateShader_TransportUsesPhysicalSkyScaleIndependentFromScreenAmbientIbl()
    {
        string shader = ReadRepoText("Njulf.Shaders", "ddgi_update_shared.glsl");
        string simpleTrace = ReadRepoText("Njulf.Shaders", "ddgi_simple_trace.comp");
        string hitShading = ReadRepoText("Njulf.Shaders", "ddgi_hit_shading.glsl");
        string simpleShared = ReadRepoText("Njulf.Shaders", "ddgi_simple_shared.glsl");
        string common = ReadRepoText("Njulf.Shaders", "common.glsl");
        string forward = ReadRepoText("Njulf.Shaders", "forward.frag");
        string pass = ReadRepoText("Njulf.Rendering", "Pipeline", "DdgiPipelinePasses.cs");
        ...
        Assert.Multiple(() =>
        {
            Assert.That(shader, Does.Contain("#include \"ddgi_hit_shading.glsl\""));
            Assert.That(simpleTrace, Does.Contain("#include \"ddgi_hit_shading.glsl\""));
            Assert.That(simpleTrace, Does.Contain("#define DDGI_HIT_USE_SELECTED_LIGHTS 0"));
            Assert.That(simpleTrace, Does.Not.Contain("SimpleHitSurface"));
            Assert.That(simpleTrace, Does.Not.Contain("EvaluateSimpleDirectDiffuse"));
            Assert.That(hitShading, Does.Contain("bool ResolveCommittedHitSurface("));
            ...
            Assert.That(hitShading, Does.Contain("currentEnvironment.Enabled == 0u"));
            Assert.That(hitShading, Does.Contain("giEnvironment.Enabled == 0u"));
            Assert.That(simpleTrace, Does.Contain("params.environmentIntensity);"));
            ...
            Assert.That(hitShading, Does.Contain("vec3 environmentRadiance = EvaluateEnvironmentRadiance("));
            Assert.That(common, Does.Contain("BindlessCubeTextures[nonuniformEXT(environment.EnvironmentTextureIndex)]"));
            Assert.That(common, Does.Contain("return max(result, vec3(0.0)) * max(environment.SkyIntensity, 0.0);"));
            Assert.That(hitShading, Does.Not.Contain("* max(environment.DiffuseIntensity, 0.0);"));
            Assert.That(simpleShared, Does.Contain("return EvaluateEnvironmentTransportIrradiance(environment, safeNormal);"));
            Assert.That(common, Does.Contain("max(environment.SkyIntensity, 0.0);"));
            Assert.That(simpleShared, Does.Not.Contain("* environment.DiffuseIntensity;"));
            Assert.That(pass, Does.Contain("float environmentIntensity = _settings.Environment.Enabled ? _settings.Environment.SkyIntensity : 0.0f;"));
    ...
    public void ForwardShader_ExposesDdgiCoverageAndCascadeDebugViews()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(shader, Does.Contain("GLOBAL_ILLUMINATION_DEBUG_DDGI_DATA_CONFIDENCE,"));
            Assert.That(shader, Does.Contain("vec3(clamp(ddgiSample.supportCoverage, 0.0, 1.0))"));
            Assert.That(shader, Does.Contain("GLOBAL_ILLUMINATION_DEBUG_DDGI_DIRECTIONAL_SUPPORT,"));
            ...
            Assert.That(shader, Does.Contain("DDGI_HIGH_OWNERSHIP_LOW_DELIVERED_INDIRECT_COUNTER"));
    ...
    public void LightCullShader_CullsAgainstTileFrustumAndVisibleDepthAndSkipsDirectionals()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(lightCull, Does.Contain("SphereOverlapsTileFrustum"));
            Assert.That(lightCull, Does.Contain("SphereOverlapsTileDepthRange"));
            Assert.That(lightCull, Does.Contain("pc.Push.CameraForward"));
            Assert.That(lightCull, Does.Contain("SharedSelectedLightIndices[128]"));
            Assert.That(lightCull, Does.Contain("InsertDeterministicLightCandidate"));
            Assert.That(lightCull, Does.Contain("WriteTiledLightHeader(tileIndex, selectedCount, tileLightOffset, overflowCount)"));
    ...
    [Test]
    public void MaskedAndFoliageCoverage_UseSharedImplementationsAcrossDepthAndForward()
    {
        string materialCoverage = ReadRepoText("Njulf.Shaders", "material_coverage.glsl");
        string depthAlpha = ReadRepoText("Njulf.Shaders", "depth_alpha.frag");
        string forward = ReadRepoText("Njulf.Shaders", "forward.frag");
        string depthMesh = ReadRepoText("Njulf.Shaders", "depth_alpha.mesh");
        string foliageCoverage = ReadRepoText("Njulf.Shaders", "foliage_coverage.glsl");
        string foliageDepth = ReadRepoText("Njulf.Shaders", "foliage_depth.frag");
        string foliageForward = ReadRepoText("Njulf.Shaders", "foliage_forward.frag");

        Assert.Multiple(() =>
        {
            Assert.That(materialCoverage, Does.Contain("material.Albedo.a * albedoSample.a * vertexAlpha"));
            Assert.That(materialCoverage, Does.Contain("MaterialCoverageSurvivesForward"));
            Assert.That(depthAlpha, Does.Contain("#include \"material_coverage.glsl\""));
            Assert.That(forward, Does.Contain("#include \"material_coverage.glsl\""));
            Assert.That(depthAlpha, Does.Contain("fragVertexColor.a"));
            Assert.That(depthMesh, Does.Contain("meshVertexColor[vertexSlot] = vertex.Color"));
            Assert.That(foliageCoverage, Does.Contain("StableFoliageCoverageDither"));
            Assert.That(foliageCoverage, Does.Contain("clusterIndex ^ (lodBand * 0x9e3779b9u)"));
            Assert.That(foliageDepth, Does.Contain("#include \"foliage_coverage.glsl\""));
            Assert.That(foliageForward, Does.Contain("#include \"foliage_coverage.glsl\""));
            Assert.That(foliageDepth, Does.Contain("FoliageCoverageSurvives("));
            Assert.That(foliageForward, Does.Contain("FoliageCoverageSurvives("));
        });
    }
