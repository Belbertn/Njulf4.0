...
internal static class Program
{
    public static int Main(string[] args)
    {
        ...
        {
            Console.WriteLine("--example model|camera|procedural|custom|content|effects|input|timing|sprites|physics-query|physics-simulation|audio [--frames N] [--validation] [--no-aa] [--async-validation] [--native-inspect] [--capture path.png] [--bindings path.json]");
            return 0;
        }
        ...
        try
        {
            ...
            if (game is CameraExample cameraExample) cameraExample.ValidateCameraCompletion();
            return 0;
        }
...
internal sealed record ExampleOptions(string Example, int Frames, bool Validation, bool NativeInspect, string? Capture)
{
    ...
    public static ExampleOptions Parse(string[] args)
    {
        ...
        if (example is not ("model" or "camera" or "procedural" or "custom" or "content" or "effects" or "input" or "timing" or "sprites" or "physics-query" or "physics-simulation" or "audio"))
