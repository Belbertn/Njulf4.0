...
internal static class AsyncComputePassCatalog
{
    ...
    [
        ...
        Graphics("AreaShadowDenoisePass", "Area visibility, depth and motion", "Forward+ area lighting", "Private per-light histories are serialized on the graphics queue."),
        Graphics("HybridReflectionClassifyPass", "Opaque receiver payload and prior reflection history", "DDGI reconstruction and the sharp-detail subsystem", "Classification owns the frame reset and publishes one compact visible-receiver tile list for every downstream reflection stage."),
        Graphics("HybridReflectionSsrPass", "Compacted reflection receivers, SceneColor, and Hi-Z", "Adjacent ray-query recovery", "SSR is the screen-space half of the sharp-detail subsystem and shares its bounded recovery queue with ray query."),
        ...
        Graphics("HybridReflectionDdgiBasePass", "Compacted reflection receivers and directional DDGI", "SSR/ray sharp-detail overlay and reflection resolve", "The material-aware DDGI base reconstructs only classified receiver tiles before the sharp-detail subsystem reuses the same compact list."),
        ...
        Graphics("OpticalLayerClearPass", "Frame slot fence", "Transparent export", "Layer history shares graphics-queue buffers."),
        Graphics("OpticalTemporalPass", "Transparent layers", "Optical spatial filter", "History is a serial graphics-queue transaction."),
        Graphics("OpticalSpatialPass", "Optical temporal history", "Optical correction", "Immediate scene-color consumer."),
        Graphics("OpticalCorrectionPass", "Filtered optical layers", "Transparency composite", "Preserves graphics attachment composition."),
    ...
    /// <summary>
    /// Auto activation is fail-closed. Preferred paths are ordered first for
    /// timing collection, but preference never substitutes for a reviewed
    /// correctness certificate or the runtime profitability gate.
    /// </summary>
