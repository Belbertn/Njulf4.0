...
public sealed class ShaderBuildTests
{
    ...
    public void ForwardShader_ScalesDdgiByCoverageAndComplementsFallback()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(shader, Does.Contain("float sparseRamp = smoothstep(0.0, 0.10, confidence);"));
            Assert.That(shader, Does.Contain("float fullRamp = smoothstep(0.10, 0.45, confidence);"));
            Assert.That(shader, Does.Contain("return sparseRamp * mix(0.35, 1.0, fullRamp);"));
            ...
            Assert.That(shader, Does.Contain("float dataTrust = DdgiSparseDataTrust(dataConfidence);"));
            ...
            Assert.That(shader, Does.Contain("? (1.0 - cacheReadiness) * (1.0 - dataTrust)"));
    ...
    public void DdgiUpdateShader_UsesStableVisibilityTexelJitterAndSolidAngleWeightsIrradiance()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(shader, Does.Contain("DDGI_TRACE_EARLY_OUT_COUNTER_BASE = 61u"));
            ...
            Assert.That(shader, Does.Contain("gl_RayFlagsOpaqueEXT | gl_RayFlagsTerminateOnFirstHitEXT | gl_RayFlagsCullBackFacingTrianglesEXT"));
    ...
    public void DdgiUpdateShader_DirectBounceIsIndependentFromEnvironmentFallback()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(shader, Does.Contain("* max(environment.SkyIntensity, 0.0);"));
    ...
    public void DdgiUpdateShader_UsesSplitScratchAndStablePublishedCache()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(scheduleScore, Does.Contain("uint warmupMaxProbeAge = max(60u, (constants.ActiveProbeCount + requestBudget - 1u) / requestBudget);"));
            Assert.That(scheduleScore, Does.Contain("bool warmedProbe = qualityAndReason.y > 0.25"));
            Assert.That(scheduleScore, Does.Contain("probeAge <= warmupMaxProbeAge;"));
