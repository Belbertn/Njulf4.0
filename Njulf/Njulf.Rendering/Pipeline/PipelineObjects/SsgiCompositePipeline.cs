...
{
    public sealed unsafe class SsgiCompositePipeline : IDisposable
    {
        ...
        public SsgiCompositePipeline(VulkanContext context, BindlessHeap bindlessHeap, Format colorFormat)
        {
            ...
            GraphicsPipelineFactory.ValidatePushConstantRange(
                ...
                "SSGI composite pass");
            _pipelineCache = GraphicsPipelineFactory.CreatePipelineCache(_context, "SSGI Composite Pipeline Cache");
        ...
        private void CreatePipelineLayout()
        {
            var pushConstantRange = new PushConstantRange
            {
                ...
                Offset = 0,
                Size = (uint)Marshal.SizeOf<GPUSsgiCompositePushConstants>()
            };
            ...
            _layout = GraphicsPipelineFactory.CreateBindlessPipelineLayout(
                ...
                _bindlessHeap,
                pushConstantRange,
                "SSGI Composite Pipeline Layout");
        }
        ...
        private void CreatePipeline(Format colorFormat)
        {
            ...
            try
            {
                ...
                fragmentModule = ShaderModuleLoader.Load(_context, "ssgi_composite.frag.spv");
                _context.SetDebugName(vertexModule.Handle, ObjectType.ShaderModule, "composite.vert.spv");
                _context.SetDebugName(fragmentModule.Handle, ObjectType.ShaderModule, "ssgi_composite.frag.spv");

                _pipeline = CreateGraphicsPipeline(vertexModule, fragmentModule, colorFormat);
                _context.SetDebugName(_pipeline.Handle, ObjectType.Pipeline, "SSGI Composite Pipeline");
            }
            finally
            {
        ...
        private VkPipeline CreateGraphicsPipeline(ShaderModule vertexModule, ShaderModule fragmentModule, Format colorFormat)
        {
            ...
            if (result != Result.Success)
                throw new VulkanException("Failed to create SSGI composite graphics pipeline", result);
